...
module "ods_aws_lambda_apis_multiple" {
  ...
  config = {
    ...
    create_s3_sqs_lambda_role = false
    ...
    lambdas = {
      "ods-etl-hc-mkt-notebook" = {
        ...
        environment_variables = {
          ...
          RDS_PROD_SECRETS_MANAGER_ID = var.rds_prod_credentials_secret_arn
          RDS_DB_NAME = "tds"
          RDS_HOST = var.rds_cluster_endpoint
          TOKEN_URL = var.token_url
          SFMC_URL = var.sfmc_url
          S3_BUCKET = var.s3_bucket
          SFMC_API_CREDENTIALS_SECRET_MANAGER_ID = var.sfmc_api_credentials_secret_arn
        }
        vpc_config = {
